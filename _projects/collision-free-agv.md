...
项目研究无人仓中的多 AGV(automatic guided vehicle) 协同调度问题。系统需要同时完成订单分配、托盘选择、拣选工位匹配和出库—回库—回收路径规划；当多台 AGV 并发运行时，还要处理节点冲突、相向冲突、局部拥堵和储位死锁。
...
先合并相同 SKU (Stock Keeping Unit)的订单，再按照托盘库存将订单拆分为子订单，使一个子订单尽量只需一次出库作业即可完成。托盘分组同时考虑组内商品种类相似度和组间商品数量均衡；随后依据历史出库趟次，将高频托盘组匹配到距离较短的储位节点和拣选工位。
